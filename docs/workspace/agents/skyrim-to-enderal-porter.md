---
id: "workspace/agents/skyrim-to-enderal-porter"
title: "skyrim-to-enderal-porter"
slug: "skyrim-to-enderal-porter"
...
source_path: ".claude/agents/skyrim-to-enderal-porter.md"
...
lines: 306
content_sha256: "ba32eed0cf1e75011d7bbd2e98b94db0cd3e149728e81074957a63c99b35ed3c"
synced_at: "2026-09-02T11:44:19Z"
sync_version: 1
agent_name: "skyrim-to-enderal-porter"
description: "Triage and port Skyrim SE mods to Enderal SE. Use FIRST, before planning or authoring anything, whenever a Skyrim mod is being brought into Enderal — it runs the cheap kill-checks (form version, SKSE build, masters, distribution, worldspace/record collisions) that decide whether a mod is portable at all and whether it needs a patch or a replacement plugin."
...
### 2.4 The mod's BSA can overwrite Enderal's own scripts

**[verified in-game 2026-08-07]** This one is invisible in the plugin. **Enderal replaces 55 vanilla
script names**, and a mod's archive loads *after* Enderal's because its plugin does — so any script
the mod ships under one of those names **wins**, and you have silently restored Skyrim behaviour to a
game built without it.

Apocalypse's BSA holds 206 compiled scripts. Two collide with Enderal's 5,029:

| Script | Enderal's | Apocalypse's |
|---|---|---|
| `dgintimidateplayerscript` | 4 lines — `; DUMMY, DO NOTHING` | the full vanilla brawl script, 2,425 bytes |
| `dgintimidatealiasscript` | 4 lines — `; DUMMY, DO NOTHING` | the full vanilla alias script, 1,983 bytes |

SureAI gutted the brawl/intimidate system deliberately. The restored scripts reach for
`dgintimidatequestscript`, `DGIntimidateFaction` and `CR04Running`, none of which exists here.

**The check is one command.** Unpack (or list) the archive and intersect the script names against
Enderal's tree:

```bash
ls <mod>/scripts/*.pex        | sed 's#.*/##; s#\.pex$##' | sort > /tmp/mod.txt
ls reference/base/EnderalScripts/source/scripts/*.psc | sed 's#.*/##; s#\.psc$##' \
                              | tr 'A-Z' 'a-z' | sort > /tmp/enderal.txt
comm -12 /tmp/mod.txt /tmp/enderal.txt
```

**The fix** is to ship Enderal's own version **loose**, since loose files beat any BSA. Copy the
`.psc` verbatim from `reference/base/EnderalScripts/`, compile with **Enderal's tree first** on the
`-i` path — first-wins, and the wrong order rebuilds the very script you are suppressing — and commit
the `.pex`. Then say on the mod page that your mod must sit **below** the ported one in MO2's file
order. A correct build of those two stubs is ~480 bytes; near 2 KB means you compiled vanilla's.

Collisions with names Enderal does *not* have are usually harmless — the quests they belong to do not
exist here either.

### 2.5 A mod that adds any navmesh carries Bethesda's whole navigation map

**[verified]** Add one navmesh — even in the mod's own interior cell — and the Creation Kit
regenerates the entire `NAVI` record, stamping the vanilla navigation map into the plugin.
Apocalypse's was 6,633 lines: 10 vanilla exterior `MapInfos` entries with thousands of merge FormIDs,
plus a `PreferredPathing` block of 6,312 references, **none** of them the mod's own.

It is built against a `Skyrim.esm` that is not the one Enderal ships, and note Enderal's own
`Skyrim.esm` NAVI has a **null FormKey** (Spriggit writes `Null.yaml`) while the real map is
`000802:Enderal - Forgotten Stories.esm` — so a plugin NAVI at a vanilla ID is *injecting*, not
overriding.

**Keep only the entries whose `NavigationMesh` FormKey belongs to the mod, and drop
`PreferredPathing` entirely.** `src/Apocalypse/tools/10-strip-vanilla-navi.ps1` does exactly that:
6,633 lines → 34.

Resolve before you panic about it, though. Of Apocalypse's 721 distinct FormIDs, **720 named
navmeshes Enderal does not have**, and all 10 `MapInfos` entries parented to worldspace `00003C`
(`MQP01Home`), so the record was very nearly inert. Strip it because shipping a foreign navigation map
is not defensible, not because you have evidence it broke something — and test it as a **regression**
check (the mod's own cells, plus any cell whose navmesh genuinely collided), not as a fix
confirmation.
...
Keeping the original filename so the mod's BSAs keep loading is the point of a replacement — but note
that is also exactly how **2.4** bites, since those archives keep overriding Enderal's scripts. The
two go together: replacement plugin, original filename, *plus* loose stubs for any colliding script.
...
BSA scripts  : 206 in archive, N collide with Enderal's 5029 [name them] -> ship stubs loose
NAVI         : present / absent; K of M FormIDs are real Enderal navmeshes
...
**And a record-level check is not a runtime check.** Two fixes on the Apocalypse port passed every
verifier, shipped, and were still wrong: a quest flag that did not stop a quest script, and a second
distribution entry point nobody had grepped for. One Papyrus log caught both. Enable logging in the
**MO2 profile's** INI (`bEnableLogging=1` *and* `bEnableTrace=1`; the copy in `Documents` is ignored)
and read `Documents\My Games\`**`Skyrim`**` Special Edition\Logs\Script\Papyrus.0.log` — Skyrim's
folder, not Enderal's, same quirk as the crash logs.
